...
(function () {
    ...
    if (window.jsxpath) {
        ...
        if (scriptElm != undefined) {
            scriptSrc = scriptElm.src;
        }
        config = {};
    ...
    this.Reinitialize = function(htmlDocument){
        install(htmlDocument);
    }

    var install = function(htmlDoc) {

        //win = win || this;
        //var doc = win.document;
        var doc = htmlDoc;
        var undefined = htmlDoc.undefined;

        doc.XPathExpression = function(expr) {
            if (!expr.length) {
                //throw htmlDoc.Error('no expression');
                AALogger('HTMLJavascriptDOMXPath', 'install', 'no expression');
            }
            ...
            if (!lexer.empty()) {
                //throw htmlDoc.Error('bad token: ' + lexer.next());
                AALogger('HTMLJavascriptDOMXPath', 'install', 'bad token: ' + lexer.next());
            }
        };

        doc.XPathExpression.prototype.evaluate = function(node, type) {
            return new htmlDoc.XPathResult(this.expr.evaluate(new Ctx(node)), type);
        };

        doc.XPathResult = function (value, type) {
            if (type == 0) {
                switch (typeof value) {
                    case 'object':  type ++; // 4
                    case 'boolean': type ++; // 3
        ...
        doc.XPathResult.prototype.iterateNext = function() { return this.nodes[this.index++] };
        doc.XPathResult.prototype.snapshotItem = function(i) { return this.nodes[i] };

        doc.XPathResult.ANY_TYPE = 0;
        doc.XPathResult.NUMBER_TYPE = 1;
        doc.XPathResult.STRING_TYPE = 2;
        doc.XPathResult.BOOLEAN_TYPE = 3;
        doc.XPathResult.UNORDERED_NODE_ITERATOR_TYPE = 4;
        doc.XPathResult.ORDERED_NODE_ITERATOR_TYPE = 5;
        doc.XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE = 6;
        doc.XPathResult.ORDERED_NODE_SNAPSHOT_TYPE = 7;
        doc.XPathResult.ANY_UNORDERED_NODE_TYPE = 8;
        doc.XPathResult.FIRST_ORDERED_NODE_TYPE = 9;


        doc.createExpression = function(expr) {
            return new htmlDoc.XPathExpression(expr, null);
        };

        doc.evaluateAAXPath = function(expr, context, _, type) {
